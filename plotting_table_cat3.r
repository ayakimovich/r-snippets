...
read_file <- "R:\\Data\\150109_particle_to_gfp\\b_optimized\\cat4_data.csv"
save_file <- "R:\\Data\\150109_particle_to_gfp\\b_optimized\\cat4.pdf"
...
plot_data <- data.frame(intensity = molten_data$intensity, time_points = time_points, particle_number = as.numeric(data_headers))
...
#for (i in 1:ncol(data_table)){
  p <- ggplot(plot_data, aes(x = plot_data$time_point, y = plot_data$intensity, group = plot_data$particle_number))
  ...
  p <- p  + scale_y_continuous(limits = c(0, 0.115), breaks= c(0.03, 0.06, 0.09)) #  + coord_cartesian(ylim = c(0, 1))
